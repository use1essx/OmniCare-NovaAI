"""
Seed KB Categories - Simple 3-Level Structure

Matches the folder structure at KB folder:
- Level 1: Age Groups (elderly, child, teen, adult)
- Level 2: Categories (e.g., elderly discount, health)
- Level 3: Topics (e.g., Elder Card, self-care)
"""
...
async def seed_categories():
    ...
    async for db in get_async_db():
        try:
            ...
            await db.execute(text("DELETE FROM document_category_tags"))
            await db.execute(text("UPDATE knowledge_documents SET category_id = NULL"))
            await db.execute(text("DELETE FROM kb_categories"))
            await db.commit()
            ...
            print(f"{'='*60}")
            print(f"  Level 1 (Age Groups): 4")
            print(f"  Level 2 (Categories): 2")
            print(f"  Level 3 (Topics): 3")
            print(f"{'='*60}")
            
            # Show tree structure
            print("\nCategory Tree Structure:")
            print("📁 Elderly (長者)")
            print("  📁 Elderly Discount (長者優惠)")
            print("    📄 Elder Card (長者卡)")
            print("  📁 Health (健康)")
            print("    📄 Self-Care (自我照顧)")
            print("    📄 Health Life (健康生活)")
            print("📁 Child (兒童)")
            print("📁 Teen (青少年)")
            print("📁 Adult (成人)")
...
if __name__ == "__main__":
    print("="*60)
    print("KB Categories Seeding - Simple 3-Level Structure")
    print("="*60)
