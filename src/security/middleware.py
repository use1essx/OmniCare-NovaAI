...
class SecurityHeadersMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        is_live2d = "/live2d" in path or path == "/" or path == "/index.html"
        
        # Build connect-src directive dynamically
        connect_sources = [
            "'self'", 
            "https://cdn.jsdelivr.net",
            "http://localhost:8790",  # Allow local STT server
            "ws://localhost:8790"     # Allow local STT websocket
        ]
        if is_live2d:
        ...
        # Permissions-Policy handling
        # Allow microphone for self (needed for voice input)
        default_permissions_policy = (
            "camera=(), microphone=(self), geolocation=(), "
            "payment=(), usb=(), magnetometer=(), gyroscope=()"
        )
        permissions_policy_value = (
            settings.live2d_permissions_policy if is_live2d else settings.default_permissions_policy
        ) or default_permissions_policy
