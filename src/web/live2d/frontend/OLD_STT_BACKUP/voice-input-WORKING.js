...
class WhisperEngine {
    ...
    async _processAudio() {
        ...
        try {
            ...
            const response = await fetch('/live2d/api/stt/stream', {
                method: 'POST',
                body: formData
            });
...
class SmartVoiceInput {
    ...
    _onEngineError(error) {
        ...
        // Try fallback to Whisper if Web Speech fails
        if (this.state.currentEngine === 'web-speech' && error.recoverable) {
            console.log('[SmartVoiceInput] Falling back to Whisper...');
    ...
    _loadSettings() {
        try {
            ...
            if (saved) {
                ...
                this.config.privacyMode = settings.privacyMode || false;
